...
def run_refinement_attempt(
    ...
    output_name = (
        f"{cfg['prefix']}refine_"
        f"{status}_{refinement_index}.json"
    )
    ...
    if result["converged"]:
        ...
        saveAntimony(output_path, os.path.splitext(output_path)[0] + "antimony.txt")
...
def run_evolution_mode(
    ...
    while True:
        ...
        if result["converged"]:
            ...
            image_path_diagram = os.path.join(success_dir, f"{cfg['prefix']}_diagram.png")
            image_path_visualize = os.path.join(success_dir, f"{cfg['prefix']}_visualize.png")
            txt_path_antimony = os.path.join(success_dir, f"{cfg['prefix']}_antimony.txt")
